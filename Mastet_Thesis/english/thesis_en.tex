...
\hypersetup{
    ...
    pdftitle={A Comparative Analysis of Classical Machine Learning Methods and Variational Quantum Methods for Breast Tumor Classification},          % title
    pdfauthor={Charisis Nikolaos},    % author
    pdfsubject={Master Thesis on Quantum Machine Learning}               % subject of the document
}
\urlstyle{same}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%% User-specific package inclusions %%%%%%%%%%%%%%%%%%%%%%%%%
...
%
\titleGr{Συγκριτική Ανάλυση Κλασικών Μεθόδων Μηχανικής Μάθησης και Mεταβλητών Kβαντικών Μεθόδων για την Ταξινόμηση Όγκων Μαστού }
\titleEn{A Comparative Analysis of Classical Machine Learning Methods and Variational Quantum Methods for Breast Tumor Classification}
...
\advisorRankGr{Δόκτωρ, Ερευνήτρια}
\advisorOrgGr{ΕΚΠΑ}
\advisorEn{Mandilara Aikaterini}
\advisorRankEn{Doctor, Researcher}
\advisorOrgEn{NKUA}
...
%
\examinationDateGr{27 Φεβρουαρίου 2026}
\examinationDateEn{27 February, 2026}

%
% Supervisors[heading(optional, defaluts to ΕΠΙΒΛΕΠΩΝ/SUPERVISOR)]{people} (greek and english)
%
\supervisorsGr[Επιβλέπουσα]{
    {\bfseries Μανδηλαρά Αικατερίνη}, Δόκτωρ, Ερευνήτρια ΕΚΠΑ\\
    % {\bfseries Επιβλέπων Δεύτερος}, Καθηγητής ΕΚΠΑ
}
\supervisorsEn[Supervisor]{
    {\bfseries Mandilara Aikaterini}, Doctor, Researcher NKUA\\
...
%
\abstractGr{%
Ο καρκίνος του μαστού αποτελεί την πιο συχνή κακοήθεια στον γυναικείο πληθυσμό παγκοσμίως, με τις ασθενείς αυτές να αντιπροσωπεύουν έως και το 36\% του συνόλου των ογκολογικών περιστατικών. Με την πάροδο των ετών, μια ευρεία ποικιλία αλγορίθμων μηχανικής μάθησης έχει επιστρατευτεί για να βοηθήσει στην πρόγνωση των ιστών του καρκίνου του μαστού. Πρόσφατα, η άνοδος της κβαντικής υπολογιστικής και των κβαντικών επεξεργαστών εισήγαγε μια πολλά υποσχόμενη νέα μέθοδο ταξινόμησης μέσω της χρήσης Μεταβλητών Κβαντικών Ταξινομητών (Variational Quantum Classifiers - VQCs).

Σε αυτή τη μελέτη, χρησιμοποιώντας δεδομένα όγκων μαστού που προέρχονται από αναρροφήσεις με λεπτή βελόνα (FNA), εξετάζουμε την απόδοση ταξινόμησης των VQCs σε διαφορετικές διαμορφώσεις κβαντικών ψηφίων (qubits). Διερευνούμε τον αντίκτυπο της εισαγωγής μιας κλασικής εκπαιδεύσιμης παραμέτρου στα κυκλώματά μας, καθώς και τον βέλτιστο αριθμό εκπαιδεύσιμων παραμέτρων που απαιτούνται για την επίτευξη κορυφαίων αποτελεσμάτων. Τα ευρήματά μας δείχνουν ότι σε διαμορφώσεις έως και 4 qubits, οι κβαντικές προσεγγίσεις είναι ανταγωνιστικές ως προς τις κλασικές μεθόδους μηχανικής μάθησης, σημειώνοντας σταθερά βαθμολογία πάνω από 90\% σε τέσσερις κύριες μετρικές. Αν και η διαμόρφωση των 5 qubits αρχίζει να υστερεί έναντι της κορυφαίας κλασικής μεθόδου, εξακολουθεί να επιτυγχάνει βαθμολογίες άνω του 88\% σε όλες τις μετρικές. Τελικά, καθίσταται σαφές ότι, παρά τις προκλήσεις σχετικά με τον χρόνο και την αποδοτικότητα της υπολογιστικής ισχύος που απαιτείται, οι VQCs στην εποχή του NISQ (Ενδιάμεσης Κλίμακας Κβαντικοί Υπολογιστές με Θόρυβο) διαθέτουν απτή προοπτική για βιοϊατρικές εργασίες γενικότερα και για την ταξινόμηση όγκων ειδικότερα.
}
%
...
\abstractEn{%
Breast cancer is the most frequent malignancy among the female population worldwide, with these patients accounting for up to 36\% of all oncological cases. Over the years, a wide variety of machine learning algorithms have been employed to assist in the prognosis of breast cancer tissues. Recently, the rise of quantum computing and quantum processors has introduced a promising new classification method through the use of Variational Quantum Classifiers (VQCs).

In this study, using breast tumor data derived from fine-needle aspirates, we examine the classification performance of VQCs across different qubit configurations. We investigate the impact of introducing a classical trainable parameter into our circuits, as well as the optimal number of trainable parameters required to achieve peak results. Our findings show that up to a 4-qubit configuration, quantum approaches are competitive with classical machine learning methods, consistently scoring above 90\% across four key metrics. While the 5-qubit configuration begins to lag behind the top-performing classical method, it still achieves scores above 88\% across all evaluated metrics. Ultimately, it becomes evident that despite challenges regarding time and computing power efficiency, VQCs in the NISQ era possess tangible potential for biomedical tasks in general and tumor classification in particular.
}
...
Machine learning is the art and science of enabling computers to learn from data to solve problems, rather than relying on explicit programming. Conversely, quantum computing involves processing information using devices governed by the laws of quantum theory. As both fields are expected to play a pivotal role in the future of information technology, exploring their integration is a natural progression. This synergy is the core focus of the emerging discipline known as Quantum Machine Learning (QML).
...
Random Forests \cite{bib:random_forest} are ensemble learning methods based on the aggregation of multiple decision trees. Each tree is trained on a bootstrap sample of the training data, while at each split only a random subset of features is considered. This dual randomness, both in data sampling and feature selection, reduces correlation among individual trees and improves overall model robustness.
...
Light Gradient Boosting Machine (LightGBM) \cite{bib:lightgbm} is a gradient boosting framework based on decision trees that is designed for high efficiency and scalability. Gradient boosting builds models sequentially, where each new tree is trained to correct the errors made by the previous ensemble. LightGBM introduces several algorithmic optimizations, including histogram-based feature binning and leaf-wise tree growth, which significantly reduce training time while maintaining high predictive accuracy.
...
\begin{itemize}
	...
	\item \textbf{Photons}: As individual particles of light, photons can encode quantum information and transmit it over long distances via optical fibers. Consequently, they are essential for applications in quantum communication and cryptography.
...
\begin{figure}[H]
	...
	\caption[Bloch sphere representation of a qubit]{Bloch sphere representation of a qubit \cite{bib:schuld_petruccione}
	}
...
Three decades passed before another scientist, John Stewart Bell, developed a method to test this phenomenon. His work ultimately provided the theoretical framework that enabled subsequent researchers to experimentally confirm the existence of quantum entanglement \cite{bib:nasa}. 
...
\begin{figure}[H]
	...
	% Place caption under the figure
	\caption[An example of a quantum circuit diagram]{An example of a quantum circuit diagram \cite{bib:bsc_thesis}
	}
	% Label the caption to include in the list of figures
...
\subsection{Input Encoding Techniques} \label{encoding}

In this section, we present several techniques for encoding or embedding classical data into an $n$-qubit quantum system. While the strategy for data encoding is of paramount importance in the field of QML, it often remains a secondary consideration in other areas of quantum computation; consequently, specialized terminology for these methods has not been widely standardized. For the purposes of this discussion, we will categorize these approaches as \textbf{basis} encoding, \textbf{amplitude} encoding, \textbf{time-evolution} encoding, and \textbf{Hamiltonian} encoding.

\subsubsection{Basis Encoding}

Basis encoding establishes a direct correspondence between an $n$-bit classical string, such as $0011$, and its equivalent computational basis state in an $n$-qubit system, denoted as $\ket{0011}$ or $\ket{3}$. This method represents the most intuitive form of data embedding, as it essentially replaces each classical bit with its corresponding qubit. A primary advantage of this approach is that it allows quantum operations to act in parallel across all bit sequences within a quantum superposition.

In scenarios where the output of an algorithm is similarly encoded into a computational basis state, the amplitudes $\alpha_i$ of each basis state serve to identify the computational result with a sufficiently high measurement probability. For instance, if the basis state $\ket{0011}$ possesses an amplitude $\alpha_3$ such that the probability $|\alpha_3|^2 > 0.5$, then repeated execution and measurement of the final state in the computational basis will identify it as the most probable outcome, representing the algorithm's result. Consequently, the primary objective of a quantum algorithm is to amplify the probability—or the squared magnitude of the amplitude—associated with the specific basis state that encodes the correct solution..

Consistent with classical computing, basis encoding utilizes a binary representation of numerical values. In this framework, a quantum state $\ket{\chi}$ for a real number $\chi \in \mathbb{R}$ corresponds to the binary representation of $\chi$ within a qubit register of size $n$. Various methods exist for translating real numbers into binary format, such as fixed-point or floating-point representations. Nevertheless, because basis encoding necessitates a high qubit count, it is generally not a practical strategy for implementation on near-term quantum hardware.

\subsubsection{Time-evolution Encoding}

Time-evolution encoding involves associating a scalar value $\chi \in \mathbb{R}$ with the temporal parameter $t$ in the unitary evolution governed by a Hamiltonian $H$. This relationship is expressed in Equation~\ref{eq:time_evol}.
...
Consequently, the state resulting from the evolution, $\ket{\psi(\chi)} = U(\chi)\ket{\psi_0}$, depends on the parameter $\chi$ in a manner determined by the Hamiltonian $H$. In the field of QML, this encoding method is frequently employed to map classical trainable parameters onto a quantum circuit. The most prevalent implementation involves Pauli rotation gates, where the Hamiltonian is defined as $H = \frac{1}{2} \sigma_\alpha$ for $\alpha \in \{x, y, z\}$. By applying a sequence of such unitary operations $U(\chi)$, one can effectively encode a real-valued vector $\chi \in \mathbb{R}^N$.

\subsubsection{Hamiltonian Encoding}

For certain applications, it is advantageous to encode matrices directly into the Hamiltonian governing a quantum system's time evolution, a technique central to the Harrow-Hassidim-Lloyd algorithm for matrix inversion \cite{bib:hhl}. This method involves mapping a square matrix $\mathbf{A}$ to a corresponding Hamiltonian $H$. In instances where $\mathbf{A}$ is not Hermitian, a standard procedure is to employ the encoding transformation detailed in Equation~\ref{eq:ham_trick} to ensure hermiticity. 
...
This approach enables the execution of computations within two distinct subspaces of the Hilbert space. Hamiltonian encoding facilitates the extraction and processing of the eigenvalues of $\mathbf{A}$, which can then be utilized to perform operations such as multiplying $\mathbf{A}$ or its inverse $\mathbf{A}^{-1}$ by an amplitude-encoded vector. The methodologies employed to apply operations governed by a specific, custom Hamiltonian are formally referred to as \textbf{Hamiltonian simulation}.

\subsubsection{Amplitude Encoding}

Amplitude encoding is significantly less prevalent in general quantum computing, as it maps classical information, such as a real-valued vector, directly onto quantum amplitudes; several methods exist for this mapping. A normalized classical vector $\mathbf{x} \in \mathbb{C}^{2^N}$, satisfying $\sum_{k} |x_k|^2 = 1$, can be represented by the probability amplitudes of a quantum state $\ket{\psi} \in \mathcal{H}$ as defined in Equation~\ref{eq:stacked_dots}:
...
\begin{equation}
	...
	\end{pmatrix}
	\leftrightarrow
	\ket{\psi_x} = \sum_{j=0}^{2^n-1} x_j\ket{j}
	\label{eq:stacked_dots}
\end{equation}

In a similar manner, a matrix $\mathbf{A} \in \mathbb{C}^{2^n \times 2^m}$ containing elements $a_{ij}$ that satisfy the normalization condition $\sum_{ij} |a_{ij}|^2 = 1$ can be represented as shown in Equation~\ref{eq:double_sum}. This encoding requires a proportional expansion of the Hilbert space to accommodate the matrix dimensions.

\begin{equation}
	\ket{\psi_A} =\sum_{i=0}^{2^m-1} \sum_{j=0}^{2^n-1} a_{ij}\ket{i}\ket{j}
	\label{eq:double_sum}
\end{equation}

This representation is particularly advantageous for quantum algorithms, as the index registers $\ket{i}$ and $\ket{j}$ correspond to the $i$-th row and $j$-th column of the matrix, respectively. By holding one register constant, it is possible to isolate and address specific rows or columns. Furthermore, for Hermitian positive matrices $\mathbf{A} \in \mathbb{C}^{2^n \times 2^n}$ with unit trace, an alternative encoding exists: the elements of the matrix can be mapped directly to the entries of a density matrix $\rho_A$, such that $a_{ij}$ corresponds to $\rho_{ij}$.

The act of encoding information within the probabilistic framework of a quantum system inherently restricts the types of mathematical transformations that can be applied to the input. This limitation is especially significant when attempting to implement nonlinear mappings of probability amplitudes, as such operations cannot be realized through unitary evolution alone, unless specialized techniques involving measurements or marginalization are employed. This constraint has been a subject of extensive discourse within the context of "nonlinear quantum theories." Research has shown that the existence of nonlinear operators would fundamentally conflict with established physical principles, which are widely accepted as cornerstones of our understanding of nature.

An additional inherent constraint of this method is the requirement that only normalized classical vectors can be processed. Consequently, quantum states represent data with one fewer dimension or degree of freedom: a classical two-dimensional vector $\begin{pmatrix} x_1 & x_2 \end{pmatrix}^\top$ must be mapped to an amplitude vector $\begin{pmatrix} \alpha_1 & \alpha_2 \end{pmatrix}^\top$ that satisfies $|\alpha_1|^2 + |\alpha_2|^2 = 1$. This restriction confines the representation to a unit circle, which is a one-dimensional manifold within a two-dimensional space. Similarly, encoding a three-dimensional vector into the amplitudes of a two-qubit system (where the fourth amplitude is set to zero) reduces the dimensionality to a spherical surface. To address this, the classical vector can be augmented with an additional element set to 1 before being normalized. Following normalization, this auxiliary element retains the information regarding the normalization constant, thereby preserving the original magnitude of the vector.

Although amplitude encoding requires significantly fewer qubits compared to basis encoding, the procedures for preparing dense amplitude vectors are computationally intensive, essentially trading spatial efficiency for temporal complexity. One such method, which we utilize for data encoding in this thesis and will discuss briefly, is Amplitude-Efficient State Preparation.

Broadly speaking, for a quantum processor with $n$ qubits, the theoretical minimum depth required for a circuit to prepare an arbitrary quantum state is established as $\frac{2^n}{n}$. Contemporary algorithms generally exceed this lower bound, requiring nearly $2^n$ operations executed in parallel. The majority of these operations consist of resource-intensive two-qubit gates.

To demonstrate a method for achieving state preparation in linear time, we consider the routine introduced by Möttönen et al. \cite{bib:mottonen}. Their approach addresses the inverse problem: transforming an arbitrary quantum state $\ket{\psi}$ into the computational ground state $\ket{0\cdots0}$. To utilize this algorithm for our objectives, we must invert every individual operation and apply the resulting sequence in reverse order.

The fundamental concept involves conditioning a rotation on qubit $q_s$ based on every possible configuration of the preceding qubits $q_1, \dots, q_{s-1}$, utilizing a sequence of \textbf{multi-controlled rotations}. Specifically, this requires performing a distinct rotation for every individual branch of the superposition, ranging from the state where all previous qubits are $\ket{0\dots0}$ to the state where they are $\ket{1\dots1}$. This process is analogous to flipping $s-1$ coins and applying a specific manipulation to the $s$-th coin that is determined by the collective outcome of those flips via a lookup table.

A complete sequence of multi-controlled rotations, defined by angles $\beta_i$ (where the specific rotation axis $x$, $y$, or $z$ remains general), entails the sequential application of $2^{s-1}$ gates. Naturally, a method is required to decompose these multi-controlled rotations into a series of elementary gates. When $s-1$ control qubits are involved, this decomposition can be achieved using $2^s$ $CNOT$ gates and $2^s$ single-qubit rotations \cite{bib:mottonen}, \cite{bib:mottonen_2}.

To prepare a general quantum state, two distinct sequences of operations must be applied, with each sequence consisting of multi-controlled rotations spanning all qubits from $q_1$ to $q_n$. The first sequence utilizes $R_z$ rotations to align the phases of the amplitudes, effectively reducing the state to a single ignorable global phase, consistent with the process of inverse state preparation. The second sequence employs $R_y$ rotations to equalize the magnitudes of all amplitudes, ultimately transforming the system into the computational ground state.

\subsection{Variational Quantum Classifier} \label{circuits}

Significant research and development have been dedicated to the creation of early-stage quantum processors, collectively referred to as NISQ devices. These systems currently operate with approximately 50 to 100 qubits and lack robust error correction mechanisms, meaning they can only provide approximate computational results. Furthermore, connectivity is limited, as not all qubits in these devices can interact directly with one another. While NISQ-era hardware serves as a theoretical platform for exploring the advantages of quantum computing, the necessity of limiting algorithms to a restricted number of qubits and gates poses a major challenge to algorithmic design. To circumvent these constraints, a specialized class of hybrid protocols known as variational quantum algorithms has been developed \cite{bib:VQA}.
...
\begin{figure}[H]
	...
	% Place caption under the figure
	\caption[General structure of a variational quantum circuit]{General structure of a variational quantum circuit \cite{bib:bsc_thesis}
	}
	% Label the caption to include in the list of figures
...
Variational quantum circuits can be conceptualized as a class of machine learning models \cite{bib:schuld_petruccione}, \cite{bib:VQA}. By applying a quantum circuit $U(x, \theta)$, which is dependent on both the input features $x$ and the trainable parameters $\theta$, to an initial state, the circuit functions as a model whose output is derived from the expectation value of an observable $O$. In the context of a classification task, this expectation value $\braket{O} \in \mathbb{R}$ is typically interpreted as the probability of the input belonging to a specific category. The discrepancy between the predicted expectation value and the actual label is subsequently utilized to define the cost function for optimization \cite{bib:Schuld_2020}, \cite{bib:VQA}.

\chapter{RESULTS FROM CLASSICAL MACHINE LEARNING METHODS} \label{mlres}

Breast cancer is the most common malignancy among women worldwide, accounting for approximately 36\% of all oncological cases \cite{bib:cancer} with numerous machine learning algorithms have been applied to support the prognosis of breast cancer tissues \cite{bib:cancer_prognosis}. Breast cancer is also the most prevalent malignant tumor worldwide, with an estimated 2.089 million women diagnosed in 2018, and its incidence continues to rise across all regions of the globe \cite{bib:cancer}. The highest incidence rates are observed in industrialized countries, a trend commonly associated with factors such as unhealthy dietary habits, nicotine use, chronic stress, and low levels of physical activity.
...
Observing the distribution of each feature, we notice that the majority of them exist on different scales. To account for this, we will scale them using appropriate methods during the application of the rnCV. 
...
Following this, we proceed to inspect the correlation between the features and the diagnosis label. Based on Pearson’s correlation coefficient, we find that 15 out of the 30 features are heavily correlated, either positively or negatively, with the class label. These features include \textit{radius mean}, \textit{perimeter mean}, \textit{area mean}, \textit{compactness mean},\textit{ concavity mean}, \textit{concave points mean}, \textit{radius se}, \textit{perimeter se}, \textit{area se}, \textit{radius worst}, \textit{perimeter worst}, \textit{area worst}, \textit{compactness worst}, \textit{concavity worst}, and \textit{concave points worst}. While we could elect to retain only these features and significantly reduce the dimensionality of the problem, we believe that in a critical task like breast cancer detection, every feature provides valuable information. 
...
Regarding the feature-wise correlation, we detect no significant linear relationships between features that are not naturally expected to be correlated. For instance, \textit{radius mean} is heavily correlated with \textit{perimeter mean}, and \textit{radius worst} shows a strong correlation with \textit{perimeter worst}. Such relationships are expected, as the radius is fundamentally used to calculate the perimeter in most circular or near-circular shapes.
...
\begin{figure}[H]
	...
	% Place caption under the figure
	\caption{Cumulative Variance our tumor data explained by PCs
	}
	% Label the caption to include in the list of figures
	\label{fig:cum_variance}
\end{figure} 

Secondly, PCA allows us to gain a general understanding of class separability. If only two PCS are utilized and no significant linear class separability is observed, it is likely that the classes are not linearly separable. 
...
Area Under the Precision–Recall Curve (PR AUC), summarizes the relationship between precision and recall across different classification thresholds. The Precision–Recall curve focuses explicitly on the performance of the positive class, making it more informative than ROC AUC when dealing with strong class imbalance. In such settings, PR AUC provides a clearer picture of how well the model identifies positive samples without generating excessive false positives. Higher PR AUC values indicate better overall performance in detecting the minority class. 
...
A central challenge in supervised machine learning is the reliable estimation of a model’s generalization performance, particularly in settings where model hyperparameters must be tuned using the available data. Hyperparameter optimization is an inherently data-driven process, and if performance is evaluated on data that has influenced this optimization, the resulting estimates are optimistically biased. This issue is especially pronounced in small or moderate-sized datasets, high-dimensional feature spaces, or when flexible models with many tunable parameters are employed. rnCV has been developed to address these challenges by providing an evaluation framework that simultaneously minimizes bias and reduces variance in performance estimation.
...
As a final step in our classical approach, we utilize the entire dataset to train and fine-tune the Logistic Regression model, from which we extract the 16 most influential features based on the descending absolute values of their coefficients. These features, in order of impact, are \textit{texture worst}, \textit{area se}, \textit{texture mean}, \textit{concave points mean}, \textit{concavity worst}, \textit{symmetry worst}, \textit{concavity mean}, \textit{compactness se}, \textit{area worst}, \textit{smoothness worst}, \textit{radius se}, \textit{symmetry se}, \textit{concave points worst}, \textit{radius worst}, \textit{perimeter worst}, and \textit{fractal dimension se}. Upon examining the coefficients, we observe that \textit{compactness se}, \textit{symmetry se}, and \textit{fractal dimension se} are negative, indicating that higher values in these features are associated with the majority class—the benign tumors. Conversely, the remaining 13 features possess positive coefficients, suggesting that higher measurements in these areas correlate with an increased likelihood of the minority class, which corresponds to malignant tumors.
...
\chapter{RESULTS FROM VARIATIONAL QUANTUM METHODS} \label{qml}

Following the determination that Logistic Regression represents the most efficacious classical machine learning approach for the current tumor classification problem, the research transitions toward the implementation of QML paradigms. Specifically, this study investigates the Variational Quantum Classifier (VQC) to identify optimal sets of trainable parameters that provide a robust and satisfactory solution to the diagnostic task. This exploration involves a systematic evaluation of the VQC's classification capabilities across architectures consisting of 1, 2, 3, 4, and 5 qubits, respectively. To facilitate this, PCA is employed to perform the necessary dimensionality reduction, ensuring the input features are structurally compatible with the constraints of the quantum circuits. The computational implementation of this framework is supported by the PennyLane \cite{bib:pennylane} Python library for quantum circuit simulation and differentiable programming, while NumPy facilitates essential numerical manipulations. Furthermore, pandas and scikit-learn are utilized for data orchestration and dimensionality reduction, while Seaborn \cite{bib:seaborn} and Matplotlib \cite{bib:matplotlib} are employed for high-fidelity data visualization and statistical plotting.
...
The second component comprises the variational segment, which incorporates the system's trainable parameters. Drawing inspiration from the work of Schuld et al.\cite{bib:Schuld_2020} and contemporary methodologies in classical neural networks, this architectural section consists of layers that are repeated for a predetermined number of iterations. Each layer is composed of a sequence of single-qubit rotation gates followed by $CNOT$ gates to facilitate multi-qubit entanglement. Adopting the notation utilized by the PennyLane library, the single-qubit rotation gates are implemented as arbitrary rotations, defined mathematically as $Rot(\phi, \theta, \omega) = R_Z(\omega) R_Y(\theta) R_Z(\phi)$. This sequence represents a rotation around the $z$-axis by an angle $\omega$, followed by a rotation around the $y$-axis by $\theta$, and a final rotation around the $z$-axis by $\phi$.
...
The third and final component of the VQC architecture is the measurement stage, during which the quantum state is collapsed to extract classical information. To maintain a streamlined and interpretable framework, the measurement is restricted to the first qubit of the circuit. The integration of $CNOT$ gates in the preceding layers is critical at this stage, as the entanglement generated by these multi-qubit operations ensures that the state of the primary qubit is inextricably linked to the states of all other qubits in the register. Consequently, the expectation value derived from the first qubit, typically obtained through measurement in the Z basis, reflects a global state influenced by the entire circuit. This approach exploits the intrinsic properties of quantum entanglement, where a measurement on a single subsystem provides representative insights into the collective quantum state of the system. An illustrative representation of a VQC configuration comprising two qubits and six layers is provided in Figure\textbf{~\ref{fig:2_qbts_6_layers}} to clarify this architectural design.
...
% Place caption under the figure
\caption{A VQC's circuit configuration comprising of two qubits and six layers
}
% Label the caption to include in the list of figures
\label{fig:2_qbts_6_layers}
\end{figure}

To derive the final predictive output of the circuit, we extract the expectation value from the measurement of the first qubit. To this result, we introduce a classical scalar known as a bias term, which is initially set to zero. Consequently, the decision function of the classifier is defined as the summation of the quantum circuit's measurement and this classical value. We distinguish between two primary configurations: the unbiased VQC, where the bias remains fixed at zero to evaluate a strictly quantum-mechanical approach, and the biased VQC, where the bias is integrated into the optimization pipeline as an additional trainable parameter.
...
To identify the optimal configuration of trainable parameters, whether strictly quantum or a hybrid of quantum and classical, we evaluate various layer depths while training the VQC over a set number of epochs, adjusted according to the qubit count. The optimization process begins with the stochastic initialization of the circuit’s variational parameters to small random values, while the classical bias is initialized at zero. Subsequently, we utilize the objective loss function and a classical optimizer to iteratively update the quantum rotation angles; in the specific case of the biased classifier, the classical bias parameter is optimized concurrently.
...
The pairplot matrix exhibits inherent symmetry relative to its principal diagonal. This structural organization implies that the bivariate relationship between any two variables is represented twice within the grid; specifically, the interaction between components $i$ and $j$ is visualized once with variable $i$ on the $y$-axis and again with the axes transposed. Although this configuration introduces a deliberate degree of visual redundancy, it significantly enhances data interpretability by providing mirrored perspectives of feature interactions. This repetition allows for a more efficient cross-comparative analysis of patterns and correlations across the various PC pairings.
...
A close examination of Figure\textbf{~\ref{fig:pairplot_2_PCs}} yields several significant insights regarding the class-specific distributions within the reduced feature space. By analyzing the marginal distributions along the main diagonal, it is evident that the two classes exhibit minimal spatial overlap with respect to the first PC, suggesting that this dimension captures features with high discriminatory power. Conversely, regarding the second PC, the probability density of the positive class appears to be largely enveloped by the distribution of the negative class. This configuration implies that while the first component provides a clear basis for separation, the second component encompasses a region of high statistical ambiguity where the feature values of the positive class are effectively a subset of those plausible for the negative class.
...
From a theoretical standpoint, the non-enveloping case corresponds to a lower Bayes error rate, resulting in a more favorable classification problem. The existence of class-specific regions leads to more balanced precision and recall, as well as generally higher discrimination metrics, such as ROC AUC. In these settings, simple models like linear classifiers are often sufficient to achieve near-optimal performance, where increasing model complexity yields only marginal improvements. Overall, the absence of enveloping behavior indicates that the feature is genuinely informative and that classification performance is primarily limited by noise rather than a fundamental overlap between the classes.

Shifting focus to the off-diagonal cells of the pairplot, we examine the bivariate relationship between the first and second PCs. It is evident that, despite localized class overlap or "spillage," the two categories appear largely linearly separable within this two-dimensional projection. This observation is particularly encouraging, as it suggests that even the most fundamental VQCs may achieve satisfactory performance. This structural behavior, including the marginal distribution overlaps, is consistent with expectations: the first component accounts for 42.34\% of the total variance, while the second describes the remaining 20.63\%. While the variance explained by each individual component typically decreases as the number of components increases, the current level of separation provides confidence that the quantum approach will yield robust results.
...
% Place caption under the figure
\caption{A single layer of the 1-qubit VQC circuit
}
% Label the caption to include in the list of figures
...
% Place caption under the figure
\caption{Performance Results for 1-qubit VQCs after 10 runs
}
% Label the caption to include in the list of figures
...
% Place caption under the figure
\caption{Efficiency Results for 1-qubit VQCs after 10 runs
}
% Label the caption to include in the list of figures
...
For the best-performing biased VQC, the circuit architecture consists of a single layer, which aligns with our previous average findings. Despite its structural simplicity, this circuit achieved an accuracy of 92.11\%, a precision of 91.70\%, a recall of 91.27\%, and an F1-score of 91.47\% at the 84th epoch.
...
The results for the best-performing unbiased VQC follow a similar trend. Consistent with our average findings, its circuit architecture consists of a single layer. This configuration achieved an accuracy of 92.11\%, a precision of 91.70\%, a recall of 91.27\%, and an F1-score of 91.47\% at the 86th epoch. 
...
The progression of key metrics for the unbiased VQC is quite similar to that of the biased case. Both curves exhibit comparable trends, and with the exception of the interval between the 80th and 100th epochs, their divergence remains largely insignificant, suggesting a lack of substantial overfitting or underfitting. The sudden increase in divergence during this specific interval coincides with the point where the cost function begins to rise. Precision appears to be the metric most affected by this development, while the others remain well within the 0.05 error margin. Interestingly, after the 100th epoch, as the cost function reaches its plateau, the performance metrics follow suit. However, this stability is not sufficient to surpass the peak scores recorded at the 86th epoch, further reinforcing the notion that while minimizing the loss function is essential to the training process, it is not an absolute panacea for achieving optimal classification results. 
...
\begin{figure}[H]
	...
	% Place caption under the figure
	\caption{A single layer of the 2-qubit VQC circuit
	}
	% Label the caption to include in the list of figures
...
\begin{figure}[H]
	...
	% Place caption under the figure
	\caption{Performance Results for 2-qubit VQCs after 10 runs
	}
	% Label the caption to include in the list of figures
...
In regard to the top performers across all circuit depths and experimental runs, the most effective unbiased classifier utilized a 6-layer circuit. This configuration reached an accuracy of 94.74\%, a precision of 94.80\%, a recall of 93.85\%, and an F1-score of 94.29\% at the 94th epoch. 
...
For the best-performing biased model, we identify a classifier with a circuit architecture consisting of 6 layers. This classifier achieves an accuracy of 95.61\%, a precision of 95.10\%, a recall of 95.54\%, and an F1-score of 95.31\% at the 75th epoch. 
...
Having concluded our investigation of the two-qubit paradigm, we proceed to increase the number of available qubits and, consequently, the number of PCs upon which the model will rely.

\section{3-qubits Results} \label{3qbts}

Since we are now working with three qubits, our data encoding scheme allows us to utilize eight features. This enables us to perform PCA with eight PCs serving as our features. With this approach, we can express 92.71\% of the variance in the training data for our classification task, which is a substantial increase compared to the 79.79\% achieved in the two-qubit case. This result was expected, as the exploratory data analysis of the classical approach had already determined that eight PCs can explain up to 92.60\% of the total variance in our data. With each of the eight components accounting for 42.34\%, 20.63\%, 10.80\%, 6.01\%, 4.65\%, 4.20\%, 2.46\%, and 1.60\% of the total explained variance respectively, we anticipate that each new component added will be less discriminatively informative than the previous ones. 
...
A closer examination of the pairplot in Figure\textbf{~\ref{fig:pairplot_8_PCs}} appears to validate this trend. Along the main diagonal, we can observe that as we progress through the PCs, their distributions exhibit increasing overlap. Specifically, in the eighth PC, the negative class distribution encapsulates the positive class almost entirely. This pattern is also evident, though to a lesser extent, in the fifth, sixth, and seventh features, which is consistent with the fact that each of these components individually accounts for less than 5\% of the total explained variance.

Inspecting the remaining scatterplot cells reveals a distinct lack of linear separability, particularly in pairs involving the newly introduced components. For instance, the pairwise scatterplot between the fourth and fifth PCs exhibits little to no separability. Interestingly, all scatterplot pairs involving the first PC appear to maintain a clear class distinction, which is a direct result of the significant amount of variance this component captures. Ultimately, we expect the classification task to become more challenging as the new features, while providing a more descriptive representation of the dataset, appear to be less informative for discriminating between classes.

To address our problem, we evaluate both biased and unbiased VQCs across configurations of 1, 5, 6, and 8 layers. These configurations involve 9, 45, 54, and 72 trainable parameters for the unbiased version, and 10, 46, 55, and 73 parameters for the biased variant, respectively. The layer architecture remains consistent with our previous models, utilizing a combination of rotation gates and $CNOT$ gates. Although several three-qubit gates are available, we maintain simplicity by using only single-qubit rotations and two-qubit $CNOT$ gates. To accommodate the increased number of trainable parameters relative to earlier experiments, we have extended the training duration to 250 epochs. 
...
\begin{figure}[H]
	...
	% Place caption under the figure
	\caption{A single layer of the 3-qubit VQC circuit
	}
	% Label the caption to include in the list of figures
...
When identifying the top-performing unbiased classifier, we report a model whose circuit consists of 5 layers. After 234 epochs, this configuration achieved an accuracy of 95.61\%, a precision of 94.83\%, a recall of 96.03\%, and an F1-score of 95.35\%. 
...
Our 4-qubit implementation allows us, through the use of the Amplitude-Efficient State Preparation routine, to encode 16 features. These features are the 16 PCs obtained by fitting the PCA transformer onto the training subset and using it to transform all three data subsets. With this strategy, we are able to explain 99.08\% of the total variance in the training data, which is slightly higher than the 98.92\% that could be explained if the transformer were fitted on the entirety of the original dataset. Each of the PCs accounts for 42.34\%, 20.63\%, 10.80\%, 6.01\%, 4.65\%, 4.20\%, 2.46\%, 1.60\%, 1.38\%, 1.27\%, 1.11\%, 0.92\%, 0.72\%, 0.39\%, 0.28\%, and 0.25\% of the total explained variance ratio, respectively. We observe that with 16 PCs, we are able to describe our problem almost in its entirety, with the drawback being that some of the newly introduced features account for only miniscule amounts of variance. 
...
As with the eight-PC case, each subsequent component becomes less informative. A close examination of the main diagonal reveals that the primary body of the positive class distribution is entirely enveloped by the negative distribution across the eight newly added components. This is expected, given that several of these components account for less than 1\% of the total explained variance ratio. Consequently, the introduction of these features is unlikely to contribute significantly to the classification of the input. This effect becomes even more apparent when focusing on the scatterplots involving these eight new features. No linear class separability is visible, as the blue negative class and the orange positive class conglomerate at the center of each corresponding cell. 
...
\begin{figure}[H]
	...
	% Place caption under the figure
	\caption{A single layer of the 4-qubit VQC circuit
	}
	% Label the caption to include in the list of figures
...
When examining the top-performing models, the best unbiased classifier features a circuit with 15 layers. At the 179th epoch, this model achieves an accuracy of 96.49\%, a precision of 96.72\%, a recall of 95.73\%, and an F1-score of 96.19\%.
...
\begin{figure}[H]
	...
	% Place caption under the figure
	\caption{A single layer of the 5-qubit VQC circuit
	}
	% Label the caption to include in the list of figures
...
Regarding the top-performing models across all iterations and layer configurations, the unbiased method identified a circuit with 20 layers that, after 321 epochs, achieved an accuracy of 88.60\%, a precision of 87.44\%, a recall of 88.99\%, and an F1-score of 88.02\%.
...
For the biased method, the best-performing classifier for the 5-qubit task utilizes a 10-layer circuit. After 198 epochs, this model achieves an accuracy of 88.60\%, a precision of 87.64\%, a recall of 88.00\%, and an F1-score of 87.81\%. 
...
\chapter{COMPARISON OF THE RESULTS} \label{final_disc}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% FROM TEMPLATE
Breast cancer remains one of the most serious and increasingly frequent malignancies worldwide. While numerous methods and complex machine learning models have been developed for the diagnosis and prognosis of these cases, there remains a need to evaluate simpler machine learning algorithms. Furthermore, the emergence of quantum computers opens up new possibilities through the utilization of VQCs.

This thesis first examines the application of six distinct machine learning approaches, LightGBM, Random Forest, Logistic Regression, SVMs, LDA, and GNB, to predict malignant samples within a population of benign tumors and masses from breast tissue. The training features for these models are derived from fine-needle aspirates and detail 30 physical characteristics of cell nuclei. Reflecting empirical data, the dataset is imbalanced, with a higher frequency of the benign class.

Following a brief exploratory data analysis and the application of rnCV, we determined that Logistic Regression was the top-performing model across nine key metrics. To gain a more robust understanding of its generalization capabilities, we performed bootstrapping with five thousand samples to calculate 95\% confidence intervals. Overall, the Logistic Regression model achieved exceptionally strong results, scoring above 95\% across all nine evaluated metrics.

Next, we approached the problem from a quantum perspective, streamlining our evaluation by focusing on four key metrics. To achieve this, we employed VQCs and tested an increasing number of qubits, ranging up to five. Because we were limited to encoding $2^n$ features for every  qubits, we had to reduce the dimensionality of our dataset. We utilized PCA to perform this reduction, resulting in specific datasets consisting of 2, 4, 8, 16, and 30 features respectively.

We then bifurcated our VQC approach into two distinct techniques: biased and unbiased. In the unbiased technique, classification results depended solely on the circuit measurement, whereas the biased technique incorporated a classical trainable parameter to influence the final output. As the number of qubits increased, we evaluated the performance of these two methods across four key metrics and assessed their overall efficiency. We also investigated how circuit depth, and the resulting increase in trainable parameters, impacted the VQCs. To ensure a robust analysis of these effects, we executed the training procedure ten times and averaged the results, while also recording the top-performing model from each set of runs.

Finally, we compared the outcomes of these two quantum techniques against one another and against the results from our top-performing classical model, Logistic Regression, using the same ten-run methodology. Regarding the quantum comparison, it was evident that both approaches yielded satisfactory results, particularly in lower dimensions where both achieved scores exceeding 95\%. Furthermore, the differences between them were not particularly stark, as any minor superiority of one over the other could be attributed to statistical error. However, an interesting finding emerged as the number of features increased: the biased approach became explicitly more efficient. It required fewer trainable parameters, and consequently less training time, to achieve results that were equal to, or better than, those of the unbiased approach. This suggests that the classical trainable parameter effectively simplified the navigation of the Hilbert space.

When compared to the classical model, however, it is evident that quantum approaches still have considerable progress to make. Specifically, while VQCs can compete with or even outperform the Logistic Regression model in low-dimensional problems, they lag significantly behind when addressing the original 30-feature problem. In that context, the quantum models are unable to surpass the 90\% threshold across any of the four metrics, whereas Logistic Regression consistently scores above 92\% in most metrics. 

This suggests that increasing the number of trainable parameters within the VQCs is necessary to achieve more comprehensive coverage of the Hilbert space. However, this adjustment raises significant practical concerns, as the current training procedures already require a substantial amount of time. Furthermore, the required computational power is a major consideration, as there is no guarantee that standard hardware can efficiently manage such demanding processes.

Ultimately, we contend that VQCs demonstrate genuine promise in addressing classification problems, even in imbalanced scenarios such as breast tumor classification. In the context of low-qubit circuits and low-dimensionality problems, the VQC approach remains competitive with well-established classical methods. However, challenges arise as the size of the Hilbert space increases. To remain comparable to classical techniques, VQCs require a significantly higher number of trainable parameters, leading to increased demands on time and resources. Furthermore, there is no guarantee that these more complex quantum models will ultimately surpass the performance of their classical counterparts.

It is certain that as quantum hardware advances and processors offer a growing number of qubits, VQC technology will evolve, demonstrating enhanced capabilities for addressing real-world challenges such as breast tumor classification. In the meantime, continued research into the application of VQCs within the NISQ era remains essential.
...
\begin{appendix}
	...
	The breast tumor data used for this thesis can be found at this \href{https://archive.ics.uci.edu/dataset/17/breast+cancer+wisconsin+diagnostic}{repository}  \cite{bib:breast_cancer_wisconsin_(diagnostic)_17} or at \href{https://www.kaggle.com/datasets/uciml/breast-cancer-wisconsin-data}{Kaggle}.
	
	The code accompanying this thesis can be found in this \href{https://github.com/vex-me-not/Master_Thesis}{Github repository}.
	All of it was run using an AMD Ryzen 5 4600G CPU with 32GB of RAM.
	

    \chapter{AI USAGE DISCLAIMER}
  
	In the preparation of this thesis, the generative artificial intelligence model \textbf{Gemini 3 Flash} by Google was utilized solely as a sophisticated editing tool to enhance the clarity and professional quality of the manuscript. Specifically, the AI was employed to identify and correct grammatical, spelling, and punctuation errors, as well as to improve the overall syntactical flow and readability of the text. It is important to emphasize that all original ideas, research methodologies, data analyses, and final conclusions presented in this work were independently conceived and executed by the author. The AI did not contribute to the intellectual content or the derivation of the results; its role was strictly limited to language refinement and proofreading of the author's original work.
