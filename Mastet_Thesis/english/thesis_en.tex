...
In this section, we present several techniques for encoding or embedding classical data into an $n$-qubit quantum system. While the strategy for data encoding is of paramount importance in the field of quantum machine learning, it often remains a secondary consideration in other areas of quantum computation; consequently, specialized terminology for these methods has not been widely standardized. For the purposes of this discussion, we will categorize these approaches as \textbf{basis} encoding, \textbf{amplitude} encoding, \textbf{time-evolution} encoding, and \textbf{Hamiltonian} encoding.
...
Consequently, the state resulting from the evolution, $\ket{\psi(\chi)} = U(\chi)\ket{\psi_0}$, depends on the parameter $\chi$ in a manner determined by the Hamiltonian $H$. In the field of quantum machine learning, this encoding method is frequently employed to map classical trainable parameters onto a quantum circuit. The most prevalent implementation involves Pauli rotation gates, where the Hamiltonian is defined as $H = \frac{1}{2} \sigma_\alpha$ for $\alpha \in \{x, y, z\}$. By applying a sequence of such unitary operations $U(\chi)$, one can effectively encode a real-valued vector $\chi \in \mathbb{R}^N$.

\subsubsection{Hamiltonian Encoding}

For certain applications, it is advantageous to encode matrices directly into the Hamiltonian governing a quantum system's time evolution, a technique central to the HHL algorithm for matrix inversion. This method involves mapping a square matrix $\mathbf{A}$ to a corresponding Hamiltonian $H$. In instances where $\mathbf{A}$ is not Hermitian, a standard procedure is to employ the encoding transformation detailed in Equation~\ref{eq:ham_trick} to ensure hermiticity. 
